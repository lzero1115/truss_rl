...
class TrussRolloutDataset:
    ...
    def batch_data_from_to(self, idx_start, idx_end):
        ...
        with torch.cuda.stream(self.tos):
            ...
            # debug info
            # print("batch length", len(compliance_batch))
...
class TrussRolloutBuffer:
    ...
    def sample_curriculum(self, num_rollouts):
        """Sample curriculum indices ensuring all curricula are sampled at least once"""
        visited = self.curriculum_visited
        rank = self.curriculum_rank.to(torch.long)  # Convert to long for indexing
        total_curricula = len(visited)
        
        # First, ensure all curricula are sampled at least once
        unvisited_count = (visited == False).sum()
        
        if unvisited_count > 0:
            # Phase 1: Sample from unvisited curricula first
            sample_weights = torch.zeros_like(visited).to(floatType)
            sample_weights[visited == False] = 1.0 / unvisited_count
            curriculum_inds = torch.multinomial(sample_weights, min(num_rollouts, unvisited_count), False)
            
            # Phase 2: If we have remaining rollouts, sample from all curricula
            remaining_rollouts = num_rollouts - len(curriculum_inds)
            if remaining_rollouts > 0:
                ...
                additional_inds = torch.multinomial(sample_weights[rank], remaining_rollouts, True)
                curriculum_inds = torch.cat([curriculum_inds, additional_inds])
        else:
            # All curricula visited - use difficulty-based sampling
            inds = torch.arange(1, rank.shape[0] + 1, device=device, dtype=floatType)
            sample_weights = torch.pow(inds, -self.per_alpha) / torch.pow(inds, -self.per_alpha).sum()
            curriculum_inds = torch.multinomial(sample_weights[rank], num_rollouts, True)

        return curriculum_inds.cpu(), unvisited_count > 0
